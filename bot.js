...
var TICKER='$FWC';
var CA='0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d';
...
loadState();
var _IMG1=path.join(__dirname,'fwc.jpg');
...
var SIL_ANG=['2-3 lines. Why hold $FWC right now.','2-3 lines. $FWC fundamentals: renounced, LP locked.','2-3 lines. Being early to $FWC.','2-3 lines. $FWC community is building.','2-3 lines. The move in $FWC is still early.'];
var silIdx=0;
...
async function doShoutout(){
  ...
  try{
    ...
    var ppt='Write 1-2 warm genuine lines appreciating these $FWC admins for keeping the community alive: '+names.join(', ')+'. Be specific, sound human, tag them by name.';
...
bot.command('socials',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('links',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
...
bot.command('shill',async function(ctx){
  var shillMsgs=[
    ...
    'Looking for a BSC token built by real people?\n\n$FWC \u2014 fully community-owned.\nRenounced contract. LP PENDING. Real narrative.\nGet in early \u261d',
    'The move others will regret missing.\n\n$FWC on BSC \u2014 community takeover.\nRenounced. LP PENDING. No rug possible.\nLoad up before it runs.',
    'What if the next gem was right here?\n\n$FWC \u2014 zero dev, 100% community.\nRenounced. LP PENDING. Low cap. Real conviction.',
    'Don\u2019t sleep on $FWC.\nCommunity took over. Dev is gone. LP PENDING.\nThis is what conviction looks like. Load up.',
  ];
...
bot.on('message',async function(ctx){
  ...
  if(admin&&!isPrivate){
    ...
    if(lower==='socials'||lower==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
    return;
  }
  if(!text)return;
  ...
  if(lower2==='socials'||lower2==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xc7bed3005277ef43f0ab87ca60ad7b8f690be33d\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
...
app.listen(PORT,async function(){console.log('$FWC bot port '+PORT);try{await new Promise(function(r){setTimeout(r,2000);});}catch(_){}try{await regWH();}catch(e){console.log(e.message);}if(parseInt(SIL_DELAY||'0')>0)try{resetSil();}catch(_){}try{schedShout();}catch(_){}setInterval(function(){if(WEBHOOK_URL)try{fetch(WEBHOOK_URL+'/health').catch(function(){});}catch(_){}},4*60*1000);console.log('$FWC bot live');});